...
export const getFeed = functions.https.onCall(async (req,res) =>{
  const docs = await admin.firestore().collection('posts').where('category', '==', 'Wipped').orderBy('date','desc').limit(20).get()
 ...
 export const getWipping = functions.https.onCall(async (req,res) =>{
   ...
   const docs = await admin.firestore().collection('posts').where('category', '==', 'Wipping').orderBy('date','desc').limit(20).get()
